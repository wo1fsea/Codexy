export type DockLocale = "en" | "zh-CN" | "ja" | "es" | "fr";

export const DEFAULT_LOCALE: DockLocale = "en";
export const LOCALE_STORAGE_KEY = "codexy.locale";

export const LOCALE_OPTIONS = [
  { value: "en", nativeLabel: "English" },
  { value: "zh-CN", nativeLabel: "简体中文" },
  { value: "ja", nativeLabel: "日本語" },
  { value: "es", nativeLabel: "Español" },
  { value: "fr", nativeLabel: "Français" }
] as const satisfies ReadonlyArray<{
  value: DockLocale;
  nativeLabel: string;
}>;

const en = {
  "language.label": "Language",
  "nav.newThread": "New thread",
  "nav.automation": "Automation",
  "nav.skills": "Skills",
  "sidebar.threads": "Threads",
  "sidebar.allProjects": "All projects",
  "sidebar.archived": "Archived",
  "sidebar.searchPlaceholder": "Search threads or projects",
  "sidebar.noThreads": "No matching threads",
  "filters.live": "Live",
  "filters.archived": "Archived",
  "filters.all": "All",
  "filters.allProjects": "All projects",
  "aria.threadArchiveFilter": "Thread archive filter",
  "aria.projectFilter": "Project filter",
  "aria.modelSelection": "Model selection",
  "aria.reasoningEffort": "Thinking level",
  "aria.approvalPolicy": "Approval policy",
  "aria.permissionPreset": "Permission mode",
  "aria.sandboxMode": "Sandbox mode",
  "aria.jumpToBottom": "Jump to bottom",
  "aria.toggleTasks": "Toggle task visibility",
  "aria.toggleProcessedSteps": "Toggle processed steps",
  "aria.language": "Language",
  "thread.untitled": "Untitled thread",
  "thread.new": "New thread",
  "stage.readyToBuild": "Ready to build in {workspace}",
  "stage.startBuilding": "Start building",
  "stage.loadingThread": "Loading thread...",
  "stage.threadTitlePlaceholder": "Thread title",
  "actions.saveTitle": "Save title",
  "actions.cancel": "Cancel",
  "actions.confirm": "Confirm",
  "actions.choosePath": "Choose path",
  "actions.usePath": "Use path",
  "actions.remove": "Remove",
  "actions.archive": "Archive",
  "actions.unarchive": "Unarchive",
  "actions.archiving": "Working...",
  "actions.compact": "Compact thread",
  "actions.fork": "Fork thread",
  "actions.review": "Review changes",
  "actions.rollback": "Rollback",
  "actions.rollbackConfirm": "Rollback last turn",
  "actions.runShellCommand": "Run command",
  "actions.send": "Send",
  "actions.shellCommand": "Run shell command",
  "actions.steer": "Send update",
  "actions.jumpToBottom": "Jump to bottom",
  "actions.takeoverAndContinue": "Take over and continue",
  "actions.allowOnce": "Allow once",
  "actions.allowForSession": "Allow for session",
  "actions.deny": "Deny",
  "actions.submitAnswers": "Submit answers",
  "actions.openLink": "Open link",
  "actions.close": "Close",
  "actions.uploadImage": "Upload image",
  "actions.stop": "Stop",
  "actions.showTerminal": "Show terminal",
  "actions.showThread": "Show thread",
  "actions.showTasks": "Show tasks",
  "actions.hideTasks": "Hide tasks",
  "thinking.label": "Thinking",
  "event.contextCompaction": "Background context auto-compacted",
  "archive.confirmArchiveTitle": "Archive thread?",
  "archive.confirmArchiveBody":
    "It will move out of the live list until you unarchive it.",
  "archive.confirmUnarchiveTitle": "Unarchive thread?",
  "archive.confirmUnarchiveBody": "It will return to the live list.",
  "archive.tooltipArchive": "Archive thread",
  "archive.tooltipUnarchive": "Unarchive thread",
  "rollback.confirmTitle": "Rollback the latest turn?",
  "rollback.confirmBody":
    "This only prunes Codex conversation history. It does not revert local file edits.",
  "shellCommand.title": "Run a thread shell command",
  "shellCommand.body":
    "This uses Codex app-server's unsandboxed ! workflow for the selected thread.",
  "shellCommand.placeholder": "git status --short",
  "takeover.detectedTitle": "This thread is active somewhere else.",
  "takeover.detectedBody":
    "Confirm to take over from the web and continue this session.",
  "takeover.notice":
    "This thread is active. Your next send will take over this session.",
  "composer.placeholder":
    "Continue this thread, or start a new thread here...",
  "status.local": "Local",
  "status.tailnet": "Tailnet",
  "status.active": "Active",
  "status.idle": "Idle",
  "status.notLoaded": "Not loaded",
  "status.systemError": "System error",
  "status.waitingOnApproval": "Waiting on approval",
  "status.commandFailed": "Failed",
  "status.commandInterrupted": "Interrupted",
  "status.commandRunning": "Running",
  "status.commandRan": "Ran",
  "status.fileEdited": "Edited",
  "status.fileAdded": "Added",
  "status.fileDeleted": "Deleted",
  "status.fileRenamed": "Renamed",
  "terminal.statusLive": "Terminal live",
  "terminal.statusStarting": "Starting terminal",
  "terminal.statusClosed": "Terminal closed",
  "terminal.statusError": "Terminal error",
  "terminal.statusIdle": "Terminal idle",
  "terminal.metaHost": "Host shell",
  "terminal.startFailed": "Failed to open terminal",
  "terminal.inputFailed": "Failed to send terminal input",
  "terminal.interruptFailed": "Failed to interrupt terminal command",
  "terminal.connectionLost": "Terminal connection lost",
  "turn.status.completed": "Completed",
  "turn.status.interrupted": "Interrupted",
  "turn.status.failed": "Failed",
  "turn.status.inProgress": "In progress",
  "turn.processed": "Processed",
  "turn.processedWithDuration": "Processed {duration}",
  "reasoningEffort.low": "Low",
  "reasoningEffort.low.description": "Faster replies with lighter reasoning",
  "reasoningEffort.medium": "Medium",
  "reasoningEffort.medium.description": "Balanced speed and reasoning depth",
  "reasoningEffort.high": "High",
  "reasoningEffort.high.description": "Deeper reasoning for harder tasks",
  "reasoningEffort.xhigh": "Max",
  "reasoningEffort.xhigh.description":
    "Maximum reasoning for the toughest tasks",
  "policies.on-request": "On request",
  "policies.untrusted": "Most actions need approval",
  "policies.on-failure": "On failure",
  "policies.never": "Never ask",
  "permissions.default": "Default permission",
  "permissions.danger-full-access": "Full access permission",
  "sandboxes.read-only": "Read only",
  "sandboxes.workspace-write": "Workspace write",
  "sandboxes.danger-full-access": "Full access",
  "request.commandApproval": "Command approval",
  "request.fileApproval": "File change approval",
  "request.userInput": "Input required",
  "request.permissionsApproval": "Permissions approval",
  "request.mcpElicitation": "MCP input required",
  "request.confirmationRequired": "Confirmation required to continue",
  "request.workingDirectory": "Working directory",
  "request.commandNeedsApproval":
    "Approval is required before running this command.",
  "request.fileNeedsApproval":
    "Confirmation is required before writing files.",
  "request.networkAccess": "Network access",
  "request.readAccess": "Read access",
  "request.writeAccess": "Write access",
  "request.serverName": "Server",
  "request.select": "Select",
  "request.processing": "Working...",
  "notice.bridgeDisconnected": "Codex bridge disconnected. Reconnecting...",
  "notice.liveReconnect": "Live stream reconnecting...",
  "error.failedLoadThread": "Failed to load thread",
  "error.initializationFailed": "Initialization failed",
  "error.refreshThreadsFailed": "Failed to refresh threads",
  "error.sendFailed": "Failed to send",
  "error.uploadFailed": "Failed to upload",
  "error.requestFailed": "Failed to submit approval",
  "error.renameFailed": "Failed to rename thread",
  "error.archiveFailed": "Failed to update archive state",
  "error.interruptFailed": "Failed to interrupt turn",
  "error.compactFailed": "Failed to compact thread",
  "error.forkFailed": "Failed to fork thread",
  "error.reviewFailed": "Failed to start review",
  "error.rollbackFailed": "Failed to rollback thread",
  "error.shellCommandFailed": "Failed to run shell command",
  "error.shellCommandRequired": "Shell command is required",
  "error.steerFailed": "Failed to send update",
  "error.networkFailed": "Network request failed",
  "error.requestFailedWithStatus": "Request failed ({status})",
  "error.listModelsFailed": "Failed to list models",
  "error.listThreadsFailed": "Failed to list threads",
  "error.promptOrAttachmentRequired":
    "Prompt or image attachment is required",
  "error.createThreadFailed": "Failed to create thread",
  "error.resolveRequestFailed": "Failed to resolve request",
  "error.readThreadFailed": "Failed to read thread",
  "error.updateThreadFailed": "Failed to update thread",
  "error.turnIdRequired": "turnId is required",
  "error.appendTurnFailed": "Failed to append turn",
  "error.noFilesUploaded": "No files uploaded",
  "error.storeFilesFailed": "Failed to store files",
  "error.uploadNotFound": "Upload not found",
  "error.invalidUploadPath": "Invalid upload path",
  "error.bridgeNotConnected": "Codex bridge is not connected",
  "error.bridgeConnectTimedOut":
    "Timed out connecting to Codex app-server",
  "error.bridgeConnectionClosed": "Codex bridge connection closed",
  "error.approvalExpired":
    "This approval request is no longer valid. Refresh the current thread and try again.",
  "error.appServerExited": "Codex app-server exited with code {code}",
  "generic.workspace": "workspace",
  "generic.image": "Image",
  "generic.imageSubtype": "{subtype} image",
  "generic.file": "File",
  "generic.fileIndexed": "File {index}",
  "generic.plan": "Plan",
  "plan.summary": "{count} tasks, {completed} completed",
  "time.minutesAgo": "{count}m ago",
  "time.hoursAgo": "{count}h ago",
  "time.daysAgo": "{count}d ago",
  "time.minutesCompact": "{count}m",
  "time.hoursCompact": "{count}h",
  "time.daysCompact": "{count}d"
} as const;

export type MessageKey = keyof typeof en;
type MessageCatalog = Record<MessageKey, string>;
export type MessageVars = Record<string, string | number>;
export type TranslateFn = (key: MessageKey, vars?: MessageVars) => string;

export const MESSAGES: Record<DockLocale, MessageCatalog> = {
  en,
  "zh-CN": {
    "language.label": "语言",
    "nav.newThread": "新线程",
    "nav.automation": "自动化",
    "nav.skills": "技能",
    "sidebar.threads": "线程",
    "sidebar.allProjects": "所有项目",
    "sidebar.archived": "已归档",
    "sidebar.searchPlaceholder": "搜索线程或项目",
    "sidebar.noThreads": "没有匹配的线程",
    "filters.live": "进行中",
    "filters.archived": "已归档",
    "filters.all": "全部",
    "filters.allProjects": "全部项目",
    "aria.threadArchiveFilter": "线程归档筛选",
    "aria.projectFilter": "项目筛选",
    "aria.modelSelection": "模型选择",
    "aria.reasoningEffort": "思考强度",
    "aria.approvalPolicy": "权限策略",
    "aria.permissionPreset": "权限",
    "aria.sandboxMode": "沙箱权限",
    "aria.jumpToBottom": "跳到底部",
    "aria.toggleTasks": "切换任务显示",
    "aria.toggleProcessedSteps": "切换处理步骤显示",
    "aria.language": "语言",
    "thread.untitled": "未命名线程",
    "thread.new": "新线程",
    "stage.readyToBuild": "准备在 {workspace} 中开始构建",
    "stage.startBuilding": "开始构建",
    "stage.loadingThread": "正在加载线程...",
    "stage.threadTitlePlaceholder": "线程标题",
    "actions.saveTitle": "保存标题",
    "actions.cancel": "取消",
    "actions.confirm": "确认",
    "actions.choosePath": "选择路径",
    "actions.usePath": "使用路径",
    "actions.remove": "移除",
    "actions.archive": "归档",
    "actions.unarchive": "取消归档",
    "actions.archiving": "处理中…",
    "actions.compact": "压缩线程",
    "actions.fork": "分叉线程",
    "actions.review": "审查变更",
    "actions.rollback": "回滚",
    "actions.rollbackConfirm": "回滚上一轮",
    "actions.runShellCommand": "运行命令",
    "actions.send": "发送",
    "actions.shellCommand": "运行 shell 命令",
    "actions.steer": "追加指令",
    "actions.jumpToBottom": "跳到底部",
    "actions.takeoverAndContinue": "接管并继续",
    "actions.allowOnce": "允许一次",
    "actions.allowForSession": "本次会话允许",
    "actions.deny": "拒绝",
    "actions.submitAnswers": "提交答案",
    "actions.openLink": "打开链接",
    "actions.close": "关闭",
    "actions.uploadImage": "上传图片",
    "actions.stop": "停止",
    "actions.showTerminal": "显示终端",
    "actions.showThread": "显示线程",
    "actions.showTasks": "展开任务",
    "actions.hideTasks": "收起任务",
    "thinking.label": "正在思考",
    "event.contextCompaction": "背景信息已自动压缩",
    "archive.confirmArchiveTitle": "归档这个线程？",
    "archive.confirmArchiveBody": "归档后它会从进行中列表移走，直到你再次取消归档。",
    "archive.confirmUnarchiveTitle": "取消归档这个线程？",
    "archive.confirmUnarchiveBody": "取消归档后它会重新回到进行中列表。",
    "archive.tooltipArchive": "归档线程",
    "archive.tooltipUnarchive": "取消归档线程",
    "rollback.confirmTitle": "要回滚最近一轮吗？",
    "rollback.confirmBody": "这只会裁剪 Codex 的会话历史，不会撤销本地文件改动。",
    "shellCommand.title": "执行线程 shell 命令",
    "shellCommand.body": "这会对当前线程使用 Codex app-server 的非沙箱 ! 工作流。",
    "shellCommand.placeholder": "git status --short",
    "takeover.detectedTitle": "检测到这个 thread 正在其他端活跃。",
    "takeover.detectedBody": "确认后会从网页端接管并继续本轮会话。",
    "takeover.notice": "这个 thread 当前处于活动状态。你下一次发送会接管当前会话。",
    "composer.placeholder": "继续 thread，或从这里开始一个新的 thread...",
    "status.local": "本地",
    "status.tailnet": "Tailnet",
    "status.active": "活动中",
    "status.idle": "空闲",
    "status.notLoaded": "未加载",
    "status.systemError": "系统错误",
    "status.waitingOnApproval": "等待批准",
    "status.commandFailed": "运行失败",
    "status.commandInterrupted": "已中断",
    "status.commandRunning": "正在运行",
    "status.commandRan": "已运行",
    "status.fileEdited": "已编辑",
    "status.fileAdded": "已新增",
    "status.fileDeleted": "已删除",
    "status.fileRenamed": "已重命名",
    "terminal.statusLive": "终端已连接",
    "terminal.statusStarting": "正在启动终端",
    "terminal.statusClosed": "终端已关闭",
    "terminal.statusError": "终端错误",
    "terminal.statusIdle": "终端空闲",
    "terminal.metaHost": "节点 Shell",
    "terminal.startFailed": "打开终端失败",
    "terminal.inputFailed": "发送终端输入失败",
    "terminal.interruptFailed": "中断终端命令失败",
    "terminal.connectionLost": "终端连接已断开",
    "turn.status.completed": "已完成",
    "turn.status.interrupted": "已中断",
    "turn.status.failed": "失败",
    "turn.status.inProgress": "进行中",
    "turn.processed": "已处理",
    "turn.processedWithDuration": "已处理 {duration}",
    "reasoningEffort.low": "低",
    "reasoningEffort.low.description": "更快回复，使用更轻量的推理",
    "reasoningEffort.medium": "中",
    "reasoningEffort.medium.description": "平衡速度与推理深度",
    "reasoningEffort.high": "高",
    "reasoningEffort.high.description": "为更复杂任务提供更深入的推理",
    "reasoningEffort.xhigh": "最高",
    "reasoningEffort.xhigh.description": "为最困难的任务提供最强推理",
    "policies.on-request": "按需请求",
    "policies.untrusted": "多数操作需批准",
    "policies.on-failure": "失败后请求",
    "policies.never": "从不请求",
    "permissions.default": "默认权限",
    "permissions.danger-full-access": "完全访问权限",
    "sandboxes.read-only": "只读",
    "sandboxes.workspace-write": "工作区可写",
    "sandboxes.danger-full-access": "完全访问",
    "request.commandApproval": "命令执行审批",
    "request.fileApproval": "文件变更审批",
    "request.userInput": "需要补充输入",
    "request.permissionsApproval": "权限审批",
    "request.mcpElicitation": "MCP 输入请求",
    "request.confirmationRequired": "需要你确认后继续",
    "request.workingDirectory": "工作目录",
    "request.commandNeedsApproval": "需要批准后才能执行命令。",
    "request.fileNeedsApproval": "需要你确认后才能继续写入文件。",
    "request.networkAccess": "网络权限",
    "request.readAccess": "读取权限",
    "request.writeAccess": "写入权限",
    "request.serverName": "服务端",
    "request.select": "请选择",
    "request.processing": "处理中…",
    "notice.bridgeDisconnected": "Codex bridge 已断开，正在重连。",
    "notice.liveReconnect": "网页实时流正在重连…",
    "error.failedLoadThread": "线程加载失败",
    "error.initializationFailed": "初始化失败",
    "error.refreshThreadsFailed": "线程刷新失败",
    "error.sendFailed": "发送失败",
    "error.uploadFailed": "上传失败",
    "error.requestFailed": "审批提交失败",
    "error.renameFailed": "重命名失败",
    "error.archiveFailed": "归档状态更新失败",
    "error.interruptFailed": "中断失败",
    "error.compactFailed": "压缩线程失败",
    "error.forkFailed": "分叉线程失败",
    "error.reviewFailed": "启动审查失败",
    "error.rollbackFailed": "回滚线程失败",
    "error.shellCommandFailed": "执行 shell 命令失败",
    "error.shellCommandRequired": "必须输入 shell 命令",
    "error.steerFailed": "追加指令失败",
    "error.networkFailed": "网络请求失败",
    "error.requestFailedWithStatus": "请求失败（{status}）",
    "error.listModelsFailed": "模型列表加载失败",
    "error.listThreadsFailed": "线程列表加载失败",
    "error.promptOrAttachmentRequired": "必须输入提示词或附加图片",
    "error.createThreadFailed": "创建线程失败",
    "error.resolveRequestFailed": "审批处理失败",
    "error.readThreadFailed": "读取线程失败",
    "error.updateThreadFailed": "更新线程失败",
    "error.turnIdRequired": "缺少 turnId",
    "error.appendTurnFailed": "追加对话失败",
    "error.noFilesUploaded": "没有上传文件",
    "error.storeFilesFailed": "保存文件失败",
    "error.uploadNotFound": "未找到上传文件",
    "error.invalidUploadPath": "上传路径无效",
    "error.bridgeNotConnected": "Codex bridge 当前未连接",
    "error.bridgeConnectTimedOut": "连接 Codex app-server 超时",
    "error.bridgeConnectionClosed": "Codex bridge 连接已关闭",
    "error.approvalExpired": "这个审批请求已经失效，请刷新当前线程后重试。",
    "error.appServerExited": "Codex app-server 已退出，退出码为 {code}",
    "generic.workspace": "工作区",
    "generic.image": "图片",
    "generic.imageSubtype": "{subtype} 图片",
    "generic.file": "文件",
    "generic.fileIndexed": "文件 {index}",
    "generic.plan": "计划",
    "plan.summary": "共 {count} 个任务，已完成 {completed} 个",
    "time.minutesAgo": "{count} 分钟前",
    "time.hoursAgo": "{count} 小时前",
    "time.daysAgo": "{count} 天前",
    "time.minutesCompact": "{count}分钟",
    "time.hoursCompact": "{count}小时",
    "time.daysCompact": "{count}天"
  },
  ja: {
    "language.label": "言語",
    "nav.newThread": "新しいスレッド",
    "nav.automation": "自動化",
    "nav.skills": "スキル",
    "sidebar.threads": "スレッド",
    "sidebar.allProjects": "すべてのプロジェクト",
    "sidebar.archived": "アーカイブ済み",
    "sidebar.searchPlaceholder": "スレッドまたはプロジェクトを検索",
    "sidebar.noThreads": "一致するスレッドはありません",
    "filters.live": "進行中",
    "filters.archived": "アーカイブ済み",
    "filters.all": "すべて",
    "filters.allProjects": "すべてのプロジェクト",
    "aria.threadArchiveFilter": "スレッドのアーカイブフィルター",
    "aria.projectFilter": "プロジェクトフィルター",
    "aria.modelSelection": "モデル選択",
    "aria.reasoningEffort": "思考レベル",
    "aria.approvalPolicy": "承認ポリシー",
    "aria.permissionPreset": "権限",
    "aria.sandboxMode": "サンドボックス権限",
    "aria.jumpToBottom": "末尾へ移動",
    "aria.toggleTasks": "タスク表示を切り替える",
    "aria.toggleProcessedSteps": "処理ステップの表示を切り替える",
    "aria.language": "言語",
    "thread.untitled": "無題のスレッド",
    "thread.new": "新しいスレッド",
    "stage.readyToBuild": "{workspace} で開始する準備ができました",
    "stage.startBuilding": "作業を開始",
    "stage.loadingThread": "スレッドを読み込み中...",
    "stage.threadTitlePlaceholder": "スレッドタイトル",
    "actions.saveTitle": "タイトルを保存",
    "actions.cancel": "キャンセル",
    "actions.confirm": "確認",
    "actions.choosePath": "パスを選択",
    "actions.usePath": "このパスを使う",
    "actions.remove": "削除",
    "actions.archive": "アーカイブ",
    "actions.unarchive": "アーカイブ解除",
    "actions.archiving": "処理中…",
    "actions.compact": "スレッドを圧縮",
    "actions.fork": "スレッドを分岐",
    "actions.review": "変更をレビュー",
    "actions.rollback": "ロールバック",
    "actions.rollbackConfirm": "直前のターンを戻す",
    "actions.runShellCommand": "コマンドを実行",
    "actions.send": "送信",
    "actions.shellCommand": "シェルコマンドを実行",
    "actions.steer": "追加入力を送信",
    "actions.jumpToBottom": "末尾へ移動",
    "actions.takeoverAndContinue": "引き継いで続行",
    "actions.allowOnce": "一度だけ許可",
    "actions.allowForSession": "このセッションで許可",
    "actions.deny": "拒否",
    "actions.submitAnswers": "回答を送信",
    "actions.openLink": "リンクを開く",
    "actions.close": "閉じる",
    "actions.uploadImage": "画像をアップロード",
    "actions.stop": "停止",
    "actions.showTerminal": "端末を表示",
    "actions.showThread": "スレッドを表示",
    "actions.showTasks": "タスクを表示",
    "actions.hideTasks": "タスクを隠す",
    "thinking.label": "考え中",
    "event.contextCompaction": "背景情報は自動的に圧縮されました",
    "archive.confirmArchiveTitle": "このスレッドをアーカイブしますか？",
    "archive.confirmArchiveBody": "再表示するまでライブ一覧から外れます。",
    "archive.confirmUnarchiveTitle": "このスレッドをアーカイブ解除しますか？",
    "archive.confirmUnarchiveBody": "解除するとライブ一覧に戻ります。",
    "archive.tooltipArchive": "スレッドをアーカイブ",
    "archive.tooltipUnarchive": "アーカイブを解除",
    "rollback.confirmTitle": "最新のターンをロールバックしますか？",
    "rollback.confirmBody":
      "Codex の会話履歴だけを巻き戻します。ローカルのファイル変更は元に戻しません。",
    "shellCommand.title": "スレッドのシェルコマンドを実行",
    "shellCommand.body":
      "選択中のスレッドに対して Codex app-server の非サンドボックス ! ワークフローを使います。",
    "shellCommand.placeholder": "git status --short",
    "takeover.detectedTitle": "このスレッドは別の場所でアクティブです。",
    "takeover.detectedBody":
      "Web から引き継いでこのセッションを続行するには確認してください。",
    "takeover.notice":
      "このスレッドは現在アクティブです。次回送信するとこのセッションを引き継ぎます。",
    "composer.placeholder":
      "このスレッドを続けるか、ここから新しいスレッドを開始...",
    "status.local": "ローカル",
    "status.tailnet": "Tailnet",
    "status.active": "アクティブ",
    "status.idle": "待機中",
    "status.notLoaded": "未読み込み",
    "status.systemError": "システムエラー",
    "status.waitingOnApproval": "承認待ち",
    "status.commandFailed": "失敗",
    "status.commandInterrupted": "中断",
    "status.commandRunning": "実行中",
    "status.commandRan": "実行済み",
    "status.fileEdited": "編集済み",
    "status.fileAdded": "追加済み",
    "status.fileDeleted": "削除済み",
    "status.fileRenamed": "名前変更済み",
    "terminal.statusLive": "端末接続中",
    "terminal.statusStarting": "端末を起動中",
    "terminal.statusClosed": "端末を閉じました",
    "terminal.statusError": "端末エラー",
    "terminal.statusIdle": "端末待機中",
    "terminal.metaHost": "ホストシェル",
    "terminal.startFailed": "端末を開けませんでした",
    "terminal.inputFailed": "端末入力の送信に失敗しました",
    "terminal.interruptFailed": "端末コマンドを中断できませんでした",
    "terminal.connectionLost": "端末接続が切断されました",
    "turn.status.completed": "完了",
    "turn.status.interrupted": "中断",
    "turn.status.failed": "失敗",
    "turn.status.inProgress": "進行中",
    "turn.processed": "処理済み",
    "turn.processedWithDuration": "処理済み {duration}",
    "reasoningEffort.low": "低",
    "reasoningEffort.low.description": "軽い推論でより速く応答します",
    "reasoningEffort.medium": "中",
    "reasoningEffort.medium.description": "速度と推論の深さのバランスを取ります",
    "reasoningEffort.high": "高",
    "reasoningEffort.high.description": "難しいタスク向けにより深く推論します",
    "reasoningEffort.xhigh": "最大",
    "reasoningEffort.xhigh.description": "最も難しいタスク向けの最大推論です",
    "policies.on-request": "必要時に確認",
    "policies.untrusted": "大半の操作で承認",
    "policies.on-failure": "失敗時に確認",
    "policies.never": "確認しない",
    "permissions.default": "デフォルト権限",
    "permissions.danger-full-access": "フルアクセス権限",
    "sandboxes.read-only": "読み取り専用",
    "sandboxes.workspace-write": "ワークスペース書き込み",
    "sandboxes.danger-full-access": "フルアクセス",
    "request.commandApproval": "コマンド承認",
    "request.fileApproval": "ファイル変更承認",
    "request.userInput": "追加入力が必要です",
    "request.permissionsApproval": "権限承認",
    "request.mcpElicitation": "MCP 入力が必要です",
    "request.confirmationRequired": "続行するには確認が必要です",
    "request.workingDirectory": "作業ディレクトリ",
    "request.commandNeedsApproval":
      "このコマンドを実行する前に承認が必要です。",
    "request.fileNeedsApproval":
      "ファイルへ書き込む前に確認が必要です。",
    "request.networkAccess": "ネットワーク権限",
    "request.readAccess": "読み取り権限",
    "request.writeAccess": "書き込み権限",
    "request.serverName": "サーバー",
    "request.select": "選択",
    "request.processing": "処理中...",
    "notice.bridgeDisconnected":
      "Codex bridge が切断されました。再接続しています...",
    "notice.liveReconnect": "ライブストリームを再接続しています...",
    "error.failedLoadThread": "スレッドの読み込みに失敗しました",
    "error.initializationFailed": "初期化に失敗しました",
    "error.refreshThreadsFailed": "スレッドの更新に失敗しました",
    "error.sendFailed": "送信に失敗しました",
    "error.uploadFailed": "アップロードに失敗しました",
    "error.requestFailed": "承認の送信に失敗しました",
    "error.renameFailed": "スレッド名の変更に失敗しました",
    "error.archiveFailed": "アーカイブ状態の更新に失敗しました",
    "error.interruptFailed": "ターンの中断に失敗しました",
    "error.compactFailed": "スレッドの圧縮に失敗しました",
    "error.forkFailed": "スレッドの分岐に失敗しました",
    "error.reviewFailed": "レビューの開始に失敗しました",
    "error.rollbackFailed": "スレッドのロールバックに失敗しました",
    "error.shellCommandFailed": "シェルコマンドの実行に失敗しました",
    "error.shellCommandRequired": "シェルコマンドを入力してください",
    "error.steerFailed": "追加入力の送信に失敗しました",
    "error.networkFailed": "ネットワークリクエストに失敗しました",
    "error.requestFailedWithStatus": "リクエストに失敗しました（{status}）",
    "error.listModelsFailed": "モデル一覧の取得に失敗しました",
    "error.listThreadsFailed": "スレッド一覧の取得に失敗しました",
    "error.promptOrAttachmentRequired":
      "プロンプトまたは画像添付が必要です",
    "error.createThreadFailed": "スレッドの作成に失敗しました",
    "error.resolveRequestFailed": "承認リクエストの処理に失敗しました",
    "error.readThreadFailed": "スレッドの読み込みに失敗しました",
    "error.updateThreadFailed": "スレッドの更新に失敗しました",
    "error.turnIdRequired": "turnId が必要です",
    "error.appendTurnFailed": "ターンの追加に失敗しました",
    "error.noFilesUploaded": "ファイルがアップロードされていません",
    "error.storeFilesFailed": "ファイルの保存に失敗しました",
    "error.uploadNotFound": "アップロードが見つかりません",
    "error.invalidUploadPath": "無効なアップロードパスです",
    "error.bridgeNotConnected": "Codex bridge は接続されていません",
    "error.bridgeConnectTimedOut":
      "Codex app-server への接続がタイムアウトしました",
    "error.bridgeConnectionClosed": "Codex bridge の接続が閉じられました",
    "error.approvalExpired":
      "この承認リクエストは無効です。現在のスレッドを更新してから再試行してください。",
    "error.appServerExited":
      "Codex app-server がコード {code} で終了しました",
    "generic.workspace": "ワークスペース",
    "generic.image": "画像",
    "generic.imageSubtype": "{subtype} 画像",
    "generic.file": "ファイル",
    "generic.fileIndexed": "ファイル {index}",
    "generic.plan": "プラン",
    "plan.summary": "{count} 件のタスク、完了 {completed} 件",
    "time.minutesAgo": "{count} 分前",
    "time.hoursAgo": "{count} 時間前",
    "time.daysAgo": "{count} 日前",
    "time.minutesCompact": "{count}分",
    "time.hoursCompact": "{count}時間",
    "time.daysCompact": "{count}日"
  },
  es: {
    "language.label": "Idioma",
    "nav.newThread": "Nuevo hilo",
    "nav.automation": "Automatización",
    "nav.skills": "Habilidades",
    "sidebar.threads": "Hilos",
    "sidebar.allProjects": "Todos los proyectos",
    "sidebar.archived": "Archivados",
    "sidebar.searchPlaceholder": "Buscar hilos o proyectos",
    "sidebar.noThreads": "No hay hilos coincidentes",
    "filters.live": "Activos",
    "filters.archived": "Archivados",
    "filters.all": "Todos",
    "filters.allProjects": "Todos los proyectos",
    "aria.threadArchiveFilter": "Filtro de archivo de hilos",
    "aria.projectFilter": "Filtro de proyecto",
    "aria.modelSelection": "Selección de modelo",
    "aria.reasoningEffort": "Nivel de razonamiento",
    "aria.approvalPolicy": "Política de aprobación",
    "aria.permissionPreset": "Permiso",
    "aria.sandboxMode": "Modo de sandbox",
    "aria.jumpToBottom": "Ir al final",
    "aria.toggleTasks": "Alternar visibilidad de tareas",
    "aria.toggleProcessedSteps": "Alternar visibilidad de pasos procesados",
    "aria.language": "Idioma",
    "thread.untitled": "Hilo sin título",
    "thread.new": "Nuevo hilo",
    "stage.readyToBuild": "Listo para empezar en {workspace}",
    "stage.startBuilding": "Empezar a construir",
    "stage.loadingThread": "Cargando hilo...",
    "stage.threadTitlePlaceholder": "Título del hilo",
    "actions.saveTitle": "Guardar título",
    "actions.cancel": "Cancelar",
    "actions.confirm": "Confirmar",
    "actions.choosePath": "Elegir ruta",
    "actions.usePath": "Usar ruta",
    "actions.remove": "Quitar",
    "actions.archive": "Archivar",
    "actions.unarchive": "Desarchivar",
    "actions.archiving": "Procesando...",
    "actions.compact": "Compactar hilo",
    "actions.fork": "Ramificar hilo",
    "actions.review": "Revisar cambios",
    "actions.rollback": "Revertir contexto",
    "actions.rollbackConfirm": "Revertir el último turno",
    "actions.runShellCommand": "Ejecutar comando",
    "actions.send": "Enviar",
    "actions.shellCommand": "Ejecutar comando shell",
    "actions.steer": "Enviar actualización",
    "actions.jumpToBottom": "Ir al final",
    "actions.takeoverAndContinue": "Tomar control y continuar",
    "actions.allowOnce": "Permitir una vez",
    "actions.allowForSession": "Permitir en esta sesión",
    "actions.deny": "Denegar",
    "actions.submitAnswers": "Enviar respuestas",
    "actions.openLink": "Abrir enlace",
    "actions.close": "Cerrar",
    "actions.uploadImage": "Subir imagen",
    "actions.stop": "Detener",
    "actions.showTerminal": "Mostrar terminal",
    "actions.showThread": "Mostrar hilo",
    "actions.showTasks": "Mostrar tareas",
    "actions.hideTasks": "Ocultar tareas",
    "thinking.label": "Pensando",
    "event.contextCompaction": "El contexto de fondo se compactó automáticamente",
    "archive.confirmArchiveTitle": "¿Archivar este hilo?",
    "archive.confirmArchiveBody": "Saldrá de la lista activa hasta que lo desarchives.",
    "archive.confirmUnarchiveTitle": "¿Desarchivar este hilo?",
    "archive.confirmUnarchiveBody": "Volverá a la lista activa.",
    "archive.tooltipArchive": "Archivar hilo",
    "archive.tooltipUnarchive": "Desarchivar hilo",
    "rollback.confirmTitle": "¿Revertir el último turno?",
    "rollback.confirmBody":
      "Esto solo recorta el historial de conversación de Codex. No revierte cambios locales en archivos.",
    "shellCommand.title": "Ejecutar un comando shell del hilo",
    "shellCommand.body":
      "Esto usa el flujo ! sin sandbox de Codex app-server para el hilo seleccionado.",
    "shellCommand.placeholder": "git status --short",
    "takeover.detectedTitle": "Este hilo está activo en otro lugar.",
    "takeover.detectedBody":
      "Confirma para tomar el control desde la web y continuar esta sesión.",
    "takeover.notice":
      "Este hilo está activo. Tu próximo envío tomará el control de esta sesión.",
    "composer.placeholder":
      "Continúa este hilo o empieza uno nuevo aquí...",
    "status.local": "Local",
    "status.tailnet": "Tailnet",
    "status.active": "Activo",
    "status.idle": "Inactivo",
    "status.notLoaded": "Sin cargar",
    "status.systemError": "Error del sistema",
    "status.waitingOnApproval": "Esperando aprobación",
    "status.commandFailed": "Falló",
    "status.commandInterrupted": "Interrumpido",
    "status.commandRunning": "En ejecución",
    "status.commandRan": "Ejecutado",
    "status.fileEdited": "Editado",
    "status.fileAdded": "Añadido",
    "status.fileDeleted": "Eliminado",
    "status.fileRenamed": "Renombrado",
    "terminal.statusLive": "Terminal activa",
    "terminal.statusStarting": "Iniciando terminal",
    "terminal.statusClosed": "Terminal cerrada",
    "terminal.statusError": "Error de terminal",
    "terminal.statusIdle": "Terminal inactiva",
    "terminal.metaHost": "Shell del host",
    "terminal.startFailed": "No se pudo abrir la terminal",
    "terminal.inputFailed": "No se pudo enviar la entrada a la terminal",
    "terminal.interruptFailed": "No se pudo interrumpir el comando de la terminal",
    "terminal.connectionLost": "Se perdió la conexión con la terminal",
    "turn.status.completed": "Completado",
    "turn.status.interrupted": "Interrumpido",
    "turn.status.failed": "Falló",
    "turn.status.inProgress": "En curso",
    "turn.processed": "Procesado",
    "turn.processedWithDuration": "Procesado {duration}",
    "reasoningEffort.low": "Bajo",
    "reasoningEffort.low.description":
      "Respuestas más rápidas con razonamiento ligero",
    "reasoningEffort.medium": "Medio",
    "reasoningEffort.medium.description":
      "Equilibrio entre velocidad y profundidad de razonamiento",
    "reasoningEffort.high": "Alto",
    "reasoningEffort.high.description":
      "Razonamiento más profundo para tareas más difíciles",
    "reasoningEffort.xhigh": "Máximo",
    "reasoningEffort.xhigh.description":
      "Razonamiento máximo para las tareas más complejas",
    "policies.on-request": "A pedido",
    "policies.untrusted": "La mayoría requiere aprobación",
    "policies.on-failure": "Al fallar",
    "policies.never": "No preguntar nunca",
    "permissions.default": "Permiso predeterminado",
    "permissions.danger-full-access": "Permiso de acceso total",
    "sandboxes.read-only": "Solo lectura",
    "sandboxes.workspace-write": "Escritura en workspace",
    "sandboxes.danger-full-access": "Acceso total",
    "request.commandApproval": "Aprobación de comando",
    "request.fileApproval": "Aprobación de cambios en archivos",
    "request.userInput": "Se requiere entrada",
    "request.permissionsApproval": "Aprobación de permisos",
    "request.mcpElicitation": "Se requiere entrada de MCP",
    "request.confirmationRequired": "Se requiere confirmación para continuar",
    "request.workingDirectory": "Directorio de trabajo",
    "request.commandNeedsApproval":
      "Se requiere aprobación antes de ejecutar este comando.",
    "request.fileNeedsApproval":
      "Se requiere confirmación antes de escribir archivos.",
    "request.networkAccess": "Acceso de red",
    "request.readAccess": "Acceso de lectura",
    "request.writeAccess": "Acceso de escritura",
    "request.serverName": "Servidor",
    "request.select": "Seleccionar",
    "request.processing": "Procesando...",
    "notice.bridgeDisconnected":
      "Codex bridge se desconectó. Reconectando...",
    "notice.liveReconnect": "Reconectando el flujo en vivo...",
    "error.failedLoadThread": "No se pudo cargar el hilo",
    "error.initializationFailed": "La inicialización falló",
    "error.refreshThreadsFailed": "No se pudieron actualizar los hilos",
    "error.sendFailed": "No se pudo enviar",
    "error.uploadFailed": "No se pudo subir",
    "error.requestFailed": "No se pudo enviar la aprobación",
    "error.renameFailed": "No se pudo renombrar el hilo",
    "error.archiveFailed": "No se pudo actualizar el estado de archivo",
    "error.interruptFailed": "No se pudo interrumpir el turno",
    "error.compactFailed": "No se pudo compactar el hilo",
    "error.forkFailed": "No se pudo ramificar el hilo",
    "error.reviewFailed": "No se pudo iniciar la revisión",
    "error.rollbackFailed": "No se pudo revertir el hilo",
    "error.shellCommandFailed": "No se pudo ejecutar el comando shell",
    "error.shellCommandRequired": "El comando shell es obligatorio",
    "error.steerFailed": "No se pudo enviar la actualización",
    "error.networkFailed": "Falló la solicitud de red",
    "error.requestFailedWithStatus": "La solicitud falló ({status})",
    "error.listModelsFailed": "No se pudieron listar los modelos",
    "error.listThreadsFailed": "No se pudieron listar los hilos",
    "error.promptOrAttachmentRequired":
      "Se requiere un prompt o un archivo de imagen adjunto",
    "error.createThreadFailed": "No se pudo crear el hilo",
    "error.resolveRequestFailed": "No se pudo resolver la aprobación",
    "error.readThreadFailed": "No se pudo leer el hilo",
    "error.updateThreadFailed": "No se pudo actualizar el hilo",
    "error.turnIdRequired": "Se requiere turnId",
    "error.appendTurnFailed": "No se pudo añadir el turno",
    "error.noFilesUploaded": "No se subió ningún archivo",
    "error.storeFilesFailed": "No se pudieron guardar los archivos",
    "error.uploadNotFound": "No se encontró la carga",
    "error.invalidUploadPath": "Ruta de carga no válida",
    "error.bridgeNotConnected": "Codex bridge no está conectado",
    "error.bridgeConnectTimedOut":
      "Se agotó el tiempo al conectar con Codex app-server",
    "error.bridgeConnectionClosed":
      "La conexión de Codex bridge se cerró",
    "error.approvalExpired":
      "Esta solicitud de aprobación ya no es válida. Actualiza el hilo actual e inténtalo de nuevo.",
    "error.appServerExited":
      "Codex app-server salió con el código {code}",
    "generic.workspace": "espacio de trabajo",
    "generic.image": "Imagen",
    "generic.imageSubtype": "Imagen {subtype}",
    "generic.file": "Archivo",
    "generic.fileIndexed": "Archivo {index}",
    "generic.plan": "Plan",
    "plan.summary": "{count} tareas, {completed} completadas",
    "time.minutesAgo": "hace {count} min",
    "time.hoursAgo": "hace {count} h",
    "time.daysAgo": "hace {count} d",
    "time.minutesCompact": "{count}m",
    "time.hoursCompact": "{count}h",
    "time.daysCompact": "{count}d"
  },
  fr: {
    "language.label": "Langue",
    "nav.newThread": "Nouveau fil",
    "nav.automation": "Automatisation",
    "nav.skills": "Compétences",
    "sidebar.threads": "Fils",
    "sidebar.allProjects": "Tous les projets",
    "sidebar.archived": "Archivés",
    "sidebar.searchPlaceholder": "Rechercher des fils ou des projets",
    "sidebar.noThreads": "Aucun fil correspondant",
    "filters.live": "Actifs",
    "filters.archived": "Archivés",
    "filters.all": "Tous",
    "filters.allProjects": "Tous les projets",
    "aria.threadArchiveFilter": "Filtre d'archivage des fils",
    "aria.projectFilter": "Filtre de projet",
    "aria.modelSelection": "Sélection du modèle",
    "aria.reasoningEffort": "Niveau de réflexion",
    "aria.approvalPolicy": "Politique d'approbation",
    "aria.permissionPreset": "Permission",
    "aria.sandboxMode": "Mode de sandbox",
    "aria.jumpToBottom": "Aller en bas",
    "aria.toggleTasks": "Basculer la visibilité des tâches",
    "aria.toggleProcessedSteps": "Basculer l'affichage des étapes traitées",
    "aria.language": "Langue",
    "thread.untitled": "Fil sans titre",
    "thread.new": "Nouveau fil",
    "stage.readyToBuild": "Prêt à démarrer dans {workspace}",
    "stage.startBuilding": "Commencer",
    "stage.loadingThread": "Chargement du fil...",
    "stage.threadTitlePlaceholder": "Titre du fil",
    "actions.saveTitle": "Enregistrer le titre",
    "actions.cancel": "Annuler",
    "actions.confirm": "Confirmer",
    "actions.choosePath": "Choisir un chemin",
    "actions.usePath": "Utiliser ce chemin",
    "actions.remove": "Retirer",
    "actions.archive": "Archiver",
    "actions.unarchive": "Désarchiver",
    "actions.archiving": "Traitement…",
    "actions.compact": "Compacter le fil",
    "actions.fork": "Dupliquer le fil",
    "actions.review": "Relire les changements",
    "actions.rollback": "Revenir en arrière",
    "actions.rollbackConfirm": "Annuler le dernier tour",
    "actions.runShellCommand": "Exécuter la commande",
    "actions.send": "Envoyer",
    "actions.shellCommand": "Exécuter une commande shell",
    "actions.steer": "Envoyer une mise à jour",
    "actions.jumpToBottom": "Aller en bas",
    "actions.takeoverAndContinue": "Prendre le relais et continuer",
    "actions.allowOnce": "Autoriser une fois",
    "actions.allowForSession": "Autoriser pour la session",
    "actions.deny": "Refuser",
    "actions.submitAnswers": "Envoyer les réponses",
    "actions.openLink": "Ouvrir le lien",
    "actions.close": "Fermer",
    "actions.uploadImage": "Téléverser une image",
    "actions.stop": "Arrêter",
    "actions.showTerminal": "Afficher le terminal",
    "actions.showThread": "Afficher le fil",
    "actions.showTasks": "Afficher les tâches",
    "actions.hideTasks": "Masquer les tâches",
    "thinking.label": "Réflexion en cours",
    "event.contextCompaction":
      "Le contexte d’arrière-plan a été compacté automatiquement",
    "archive.confirmArchiveTitle": "Archiver ce fil ?",
    "archive.confirmArchiveBody": "Il quittera la liste active jusqu’à ce que vous le désarchiviez.",
    "archive.confirmUnarchiveTitle": "Désarchiver ce fil ?",
    "archive.confirmUnarchiveBody": "Il reviendra dans la liste active.",
    "archive.tooltipArchive": "Archiver le fil",
    "archive.tooltipUnarchive": "Désarchiver le fil",
    "rollback.confirmTitle": "Annuler le dernier tour ?",
    "rollback.confirmBody":
      "Cela ne retire que l'historique de conversation Codex. Les modifications locales de fichiers ne sont pas annulées.",
    "shellCommand.title": "Exécuter une commande shell du fil",
    "shellCommand.body":
      "Cela utilise le flux ! non sandboxé de Codex app-server pour le fil sélectionné.",
    "shellCommand.placeholder": "git status --short",
    "takeover.detectedTitle": "Ce fil est actif ailleurs.",
    "takeover.detectedBody":
      "Confirmez pour reprendre depuis le web et continuer cette session.",
    "takeover.notice":
      "Ce fil est actif. Votre prochain envoi reprendra cette session.",
    "composer.placeholder":
      "Continuez ce fil ou démarrez-en un nouveau ici...",
    "status.local": "Local",
    "status.tailnet": "Tailnet",
    "status.active": "Actif",
    "status.idle": "Inactif",
    "status.notLoaded": "Non chargé",
    "status.systemError": "Erreur système",
    "status.waitingOnApproval": "En attente d'approbation",
    "status.commandFailed": "Échec",
    "status.commandInterrupted": "Interrompu",
    "status.commandRunning": "En cours",
    "status.commandRan": "Exécuté",
    "status.fileEdited": "Modifié",
    "status.fileAdded": "Ajouté",
    "status.fileDeleted": "Supprimé",
    "status.fileRenamed": "Renommé",
    "terminal.statusLive": "Terminal actif",
    "terminal.statusStarting": "Démarrage du terminal",
    "terminal.statusClosed": "Terminal fermé",
    "terminal.statusError": "Erreur du terminal",
    "terminal.statusIdle": "Terminal inactif",
    "terminal.metaHost": "Shell hôte",
    "terminal.startFailed": "Impossible d'ouvrir le terminal",
    "terminal.inputFailed": "Impossible d'envoyer l'entrée du terminal",
    "terminal.interruptFailed": "Impossible d'interrompre la commande du terminal",
    "terminal.connectionLost": "Connexion du terminal perdue",
    "turn.status.completed": "Terminé",
    "turn.status.interrupted": "Interrompu",
    "turn.status.failed": "Échec",
    "turn.status.inProgress": "En cours",
    "turn.processed": "Traité",
    "turn.processedWithDuration": "Traité {duration}",
    "reasoningEffort.low": "Faible",
    "reasoningEffort.low.description":
      "Réponses plus rapides avec un raisonnement léger",
    "reasoningEffort.medium": "Moyen",
    "reasoningEffort.medium.description":
      "Équilibre entre vitesse et profondeur de raisonnement",
    "reasoningEffort.high": "Élevé",
    "reasoningEffort.high.description":
      "Raisonnement plus approfondi pour les tâches difficiles",
    "reasoningEffort.xhigh": "Max",
    "reasoningEffort.xhigh.description":
      "Raisonnement maximal pour les tâches les plus complexes",
    "policies.on-request": "À la demande",
    "policies.untrusted": "La plupart demandent une approbation",
    "policies.on-failure": "En cas d'échec",
    "policies.never": "Ne jamais demander",
    "permissions.default": "Permission par défaut",
    "permissions.danger-full-access": "Permission d'accès complet",
    "sandboxes.read-only": "Lecture seule",
    "sandboxes.workspace-write": "Écriture workspace",
    "sandboxes.danger-full-access": "Accès complet",
    "request.commandApproval": "Approbation de commande",
    "request.fileApproval": "Approbation de modification de fichier",
    "request.userInput": "Saisie requise",
    "request.permissionsApproval": "Approbation des permissions",
    "request.mcpElicitation": "Entrée MCP requise",
    "request.confirmationRequired":
      "Une confirmation est requise pour continuer",
    "request.workingDirectory": "Répertoire de travail",
    "request.commandNeedsApproval":
      "Une approbation est requise avant d'exécuter cette commande.",
    "request.fileNeedsApproval":
      "Une confirmation est requise avant d'écrire dans les fichiers.",
    "request.networkAccess": "Accès réseau",
    "request.readAccess": "Accès en lecture",
    "request.writeAccess": "Accès en écriture",
    "request.serverName": "Serveur",
    "request.select": "Sélectionner",
    "request.processing": "Traitement...",
    "notice.bridgeDisconnected":
      "Codex bridge est déconnecté. Reconnexion...",
    "notice.liveReconnect": "Reconnexion du flux en direct...",
    "error.failedLoadThread": "Impossible de charger le fil",
    "error.initializationFailed": "Échec de l'initialisation",
    "error.refreshThreadsFailed":
      "Impossible d'actualiser les fils",
    "error.sendFailed": "Échec de l'envoi",
    "error.uploadFailed": "Échec du téléversement",
    "error.requestFailed": "Échec de l'envoi de l'approbation",
    "error.renameFailed": "Impossible de renommer le fil",
    "error.archiveFailed":
      "Impossible de mettre à jour l'état d'archivage",
    "error.interruptFailed": "Impossible d'interrompre le tour",
    "error.compactFailed": "Impossible de compacter le fil",
    "error.forkFailed": "Impossible de dupliquer le fil",
    "error.reviewFailed": "Impossible de lancer la revue",
    "error.rollbackFailed": "Impossible d'annuler le fil",
    "error.shellCommandFailed": "Impossible d'exécuter la commande shell",
    "error.shellCommandRequired": "La commande shell est requise",
    "error.steerFailed": "Impossible d'envoyer la mise à jour",
    "error.networkFailed": "La requête réseau a échoué",
    "error.requestFailedWithStatus": "La requête a échoué ({status})",
    "error.listModelsFailed": "Impossible de lister les modèles",
    "error.listThreadsFailed": "Impossible de lister les fils",
    "error.promptOrAttachmentRequired":
      "Une invite ou une image jointe est requise",
    "error.createThreadFailed": "Impossible de créer le fil",
    "error.resolveRequestFailed": "Impossible de traiter l'approbation",
    "error.readThreadFailed": "Impossible de lire le fil",
    "error.updateThreadFailed": "Impossible de mettre à jour le fil",
    "error.turnIdRequired": "turnId est requis",
    "error.appendTurnFailed": "Impossible d'ajouter le tour",
    "error.noFilesUploaded": "Aucun fichier n'a été téléversé",
    "error.storeFilesFailed": "Impossible d'enregistrer les fichiers",
    "error.uploadNotFound": "Téléversement introuvable",
    "error.invalidUploadPath": "Chemin de téléversement invalide",
    "error.bridgeNotConnected": "Codex bridge n'est pas connecté",
    "error.bridgeConnectTimedOut":
      "Le délai de connexion à Codex app-server est dépassé",
    "error.bridgeConnectionClosed":
      "La connexion de Codex bridge a été fermée",
    "error.approvalExpired":
      "Cette demande d'approbation n'est plus valide. Actualisez le fil actuel puis réessayez.",
    "error.appServerExited":
      "Codex app-server s'est arrêté avec le code {code}",
    "generic.workspace": "espace de travail",
    "generic.image": "Image",
    "generic.imageSubtype": "Image {subtype}",
    "generic.file": "Fichier",
    "generic.fileIndexed": "Fichier {index}",
    "generic.plan": "Plan",
    "plan.summary": "{count} tâches, {completed} terminées",
    "time.minutesAgo": "il y a {count} min",
    "time.hoursAgo": "il y a {count} h",
    "time.daysAgo": "il y a {count} j",
    "time.minutesCompact": "{count}m",
    "time.hoursCompact": "{count}h",
    "time.daysCompact": "{count}j"
  }
};

export function resolveLocale(input: string | null | undefined): DockLocale {
  const normalized = input?.trim().toLowerCase();

  if (!normalized) {
    return DEFAULT_LOCALE;
  }

  if (normalized.startsWith("zh")) {
    return "zh-CN";
  }

  if (normalized.startsWith("ja")) {
    return "ja";
  }

  if (normalized.startsWith("es")) {
    return "es";
  }

  if (normalized.startsWith("fr")) {
    return "fr";
  }

  return "en";
}

export function resolveLocaleFromNavigator(
  languages: readonly string[] | undefined
): DockLocale {
  for (const language of languages ?? []) {
    const resolved = resolveLocale(language);
    if (resolved !== "en" || language.toLowerCase().startsWith("en")) {
      return resolved;
    }
  }

  return DEFAULT_LOCALE;
}

export function getIntlLocale(locale: DockLocale) {
  switch (locale) {
    case "zh-CN":
      return "zh-CN";
    case "ja":
      return "ja-JP";
    case "es":
      return "es-ES";
    case "fr":
      return "fr-FR";
    default:
      return "en-US";
  }
}

export function translate(
  locale: DockLocale,
  key: MessageKey,
  vars?: MessageVars
) {
  const template = MESSAGES[locale][key] ?? en[key];
  if (!vars) {
    return template;
  }

  return template.replace(/\{(\w+)\}/g, (_, name: string) =>
    String(vars[name] ?? `{${name}}`)
  );
}
